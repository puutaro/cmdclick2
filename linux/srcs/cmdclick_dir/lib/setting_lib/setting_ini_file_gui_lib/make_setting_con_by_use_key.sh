...
make_setting_con_by_use_key(){
	...
	BEGIN {
		...
		setting_num_default_value_cons_of["'${CMDCLICK_INI_FORM_FONT_SIZE}'"]="'${FONT_SIZE_DEFAULT_NUM_VALUE}'"
		setting_num_default_value_cons_of["'${CMDCLICK_INI_LIST_FONT_SIZE}'"]="'${FONT_SIZE_DEFAULT_NUM_VALUE}'"
		setting_num_default_value_cons_of["'${CMDCLICK_INI_FORM_PADDING}'"]="'${PADDING_DEFAULT_NUM_VALUE}'"
		setting_num_default_value_cons_of["'${CMDCLICK_INI_LIST_PADDING}'"]="'${PADDING_DEFAULT_NUM_VALUE}'"
	}
	{
		...
		target_num_default_value = setting_num_default_value_cons_of[current_key]
		if(target_num_default_value) {
			sub(\
				/^[0-9.]*/, \
				current_value, \
				target_num_default_value \
			)
		    delete_processed_key(\
				current_key \
			)
			printf "--field="current_key":NUM\t"target_num_default_value"\t"
			next
		}
