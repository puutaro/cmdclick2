...
. "${INPUT_GUI_LIB_DIR_PATH}/terminal_forcus_set.sh"
. "${INPUT_GUI_LIB_DIR_PATH}/judge_signal_code.sh"
. "${INPUT_GUI_LIB_DIR_PATH}/launcher_cmd_index_by_lxterminal.sh"
. "${INPUT_GUI_LIB_DIR_PATH}/echo_x_posi_y_posi_scale_width_scale_height.sh"
...
reload_cmd(){
	LANG=C
	local inc_num=$(\
		cat "${CMDCLICK_CONF_INC_CMD_PATH}" \
			| fetch_parameter_from_pip "${GREP_INC_NUM}" \
	)
	INI_FILE_DIR_PATH=$(\
...
input_cmd_index(){
	...
	local return_status=$?
	local IFS=$'\t'
	local ini_file_and_dir_list=($(cat "${CMDCLICK_VALUE_SIGNAL_FILE_PATH}"))
	local IFS="${ifs_old}"
	cat /dev/null > "${CMDCLICK_VALUE_SIGNAL_FILE_PATH}" &
	EXECUTE_FILE_NAME=$(\
		echo_by_remove_pre_or_suffix_single_quote \
			"${ini_file_and_dir_list[0]:-}"\
	)
	INI_FILE_DIR_PATH=$(\
		echo_by_remove_pre_or_suffix_single_quote \
			"${ini_file_and_dir_list[1]:-}"\
	)
