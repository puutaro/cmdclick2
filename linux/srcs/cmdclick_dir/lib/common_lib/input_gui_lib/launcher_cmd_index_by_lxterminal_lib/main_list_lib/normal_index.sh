...
normal_index(){
	...
	echo "${ini_file_list}" | \
		fzf --delimiter $'\t' \
			...
			--bind "alt-p:execute(echo \"${SETTING_CODE},{1},{2}\" > '${CMDCLICK_VALUE_SIGNAL_FILE_PATH}')+abort" \
			...
			--bind "alt-a:reload(export IMPORT_CMDCLICK_VAL=1 && . ${IMPORT_PATH_EXEC_CMDCLICK} && . ${IMPORT_PATH_INPUT_GUI} && export SIGNAL_CODE=${SIGNAL_CODE} && exec_dec && reload_cmd)" \
