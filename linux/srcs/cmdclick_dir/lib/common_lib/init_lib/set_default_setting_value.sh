...
set_default_setting_value(){
	if [ ! -e "${CMDCLICK_INI_SETTING_FILE_PATH}" ]; then
		...
		CMDCLICK_FORM_FONT_SIZE="${CMDCLICK_INI_FONT_SIZE_DEFAULT_VALUE}"
		CMDCLICK_LIST_FONT_SIZE="${CMDCLICK_INI_FONT_SIZE_DEFAULT_VALUE}"
		CMDCLICK_FORM_PADDING="${CMDCLICK_INI_PADDING_DEFAULT_VALUE}"
	else 
		local setting_con="$(\
		...
		CMDCLICK_FORM_FONT_SIZE=$(
			fetch_parameter \
				"${setting_con}" \
				"${CMDCLICK_INI_FORM_FONT_SIZE}" \
		)
		CMDCLICK_LIST_FONT_SIZE=$(
			fetch_parameter \
				"${setting_con}" \
				"${CMDCLICK_INI_LIST_FONT_SIZE}" \
		)
		CMDCLICK_FORM_PADDING=$(
			fetch_parameter \
				"${setting_con}" \
				"${CMDCLICK_INI_FORM_PADDING}" \
		)
		CMDCLICK_LIST_PADDING=$(
			fetch_parameter \
				"${setting_con}" \
	...
	case "${CMDCLICK_FORM_FONT_SIZE:-}" in
		"") CMDCLICK_FORM_FONT_SIZE="${CMDCLICK_INI_FONT_SIZE_DEFAULT_VALUE}" ;;
  esac
	case "${CMDCLICK_LIST_FONT_SIZE:-}" in
		"") CMDCLICK_LIST_FONT_SIZE="${CMDCLICK_INI_FONT_SIZE_DEFAULT_VALUE}" ;;
	esac
	case "${CMDCLICK_FORM_PADDING:-}" in
		"") CMDCLICK_FORM_PADDING="${CMDCLICK_INI_PADDING_DEFAULT_VALUE}" ;;
	esac
	case "${CMDCLICK_LIST_PADDING:-}" in
		"") CMDCLICK_LIST_PADDING="${CMDCLICK_INI_PADDING_DEFAULT_VALUE}" ;;
	esac
}